...
import { configureTestomatio, loginToTestomatio, fetchFromTestomatio, putToTestomatio, uploadFile, getTestomatioEndpoints } from './testomatio.js';
...
if (!TESTRAIL_URL || !TESTRAIL_SESSION) {
  throw new Error('Missing required environment variables: TESTRAIL_URL and TESTRAIL_SESSION');
}
...
async function downloadAttachmentFromTestRail(attachmentPath, filename = '') {
  try {
    ...
    const tempFilePath = path.join(tmpdir(), `fix-attachment-${RUN_TIME}-${filename}`);
...
async function searchTestsWithOrphanedAttachments() {
  try {
    ...
    // Search for tests containing the attachment URL pattern
    const searchQuery = 'index.php?/attachments/get/';
    let allTests = [];
    let currentPage = 1;
    let totalPages = 1;
    
    do {
      const searchEndpoint = `${postTestEndpoint}?query=${encodeURIComponent(searchQuery)}&page=${currentPage}`;
      
      logData(`Searching for tests with query: ${searchQuery} (page ${currentPage})`);
      const response = await fetchFromTestomatio(searchEndpoint);
      ...
      allTests = allTests.concat(response.data);
      
      // Update pagination info
      if (response.meta) {
        totalPages = response.meta.total_pages || 1;
        logData(`Page ${currentPage} of ${totalPages}, found ${response.data.length} tests (${response.meta.num} total)`);
      }
...
async function fixTestAttachments(test) {
  try {
    ...
    console.log(`Found ${attachmentUrls.length} attachment URLs in test: ${title}`);
    
    if (DRY_RUN) {
      console.log(`[DRY RUN] Would process ${attachmentUrls.length} attachments:`);
    ...
    for (const attachmentUrl of attachmentUrls) {
      try {
        ...
        const filePath = await downloadAttachmentFromTestRail(attachmentUrl, filename);
        ...
        if (newUrl) {
          ...
          updatedDescription = updatedDescription.replaceAll(attachmentUrl, newUrl);
          hasUpdates = true;
          logData(`Replaced ${attachmentUrl} with ${newUrl}`);
        } else {
          console.log(`Failed to upload attachment to Testomat.io: ${attachmentUrl}`);
        }
...
async function migrateAttachments() {
  try {
    ...
    if (DRY_RUN) {
      console.log('🔍 DRY RUN MODE - No changes will be made, analyzing only...\n');
    }
    ...
    if (testsWithAttachments.length === 0) {
      console.log('No tests found with actual orphaned attachment URLs in descriptions');
      return;
    }
    ...
    console.log(`Found ${testsWithAttachments.length} tests with ${totalAttachments} orphaned attachments`);
    
    if (DRY_RUN) {
      console.log('\n=== DRY RUN ANALYSIS ===');
      console.log(`📊 ${testsWithAttachments.length} tests would be updated with ${totalAttachments} attachment(s) migrated`);
      console.log('🚀 Remove --dry-run flag to execute actual migration');
      return;
    }
    
    console.log(`Processing ${testsWithAttachments.length} tests with orphaned attachments...`);
